# cluelesscloset — Architecture & Decision Log

## Services

> Since 2026-09-23 the stack spans **two** boxes: the webapp runs on 187, and the
try-on renderer runs on 202. Only `webapp` (+ optional `ollama`) come from this
repo's `docker-compose.yml`.

| Service | Role | Port | GPU | VRAM |
|---|---|---|---|---|
| `webapp` | FastAPI app (recommend, weather, tryon, chat) | 28085 on 187 (default 28082, 0.0.0.0 for LAN) | no | — |
| `qwen-comfy` | **Renderer host** — Qwen-Image-2.1 try-on + outfit refine. Separate install (`~/qwen-image` on 202), NOT in this compose file | 8188 on 202 | yes | ~13GB |
| `ollama` | LLM stylist (phase 3) | 28114 (internal) | yes | ~3–3.5GB |

> 202 is a busy box — VS Code occupies 28082/28188/28189, so the running stack uses 28085 (187) and 8188 (202).

## Decision log

| # | Decision | Date | Rationale / notes |
|---|---|---|---|
| 1 | CatVTON over IDM-VTON/OOTD/FLUX for MVP | 2026-08-21 | <8GB VRAM @1024×768, garment-image-only input, official ComfyUI workflow. IDM-VTON needs full 16GB (solo quality mode); FLUX needs GGUF quant (~12–16GB) → phase 4 upgrade path. **SUPERSEDED by #24** |
...
| 4 | ComfyUI official CUDA image + baked CatVTON node | 2026-08-21 | Custom-node install baked into image build → target machine needs zero manual steps (portability requirement). **SUPERSEDED by #24** |
...
| 13 | Try-on chat re-render + Saved-image mode | 2026-08-21 | After a render, Enter sends a note → `/api/tryon/outfit` with `base_result` re-renders from the last image. **Saved-image mode** picks a saved outfit render (no look needed); empty `garment_ids` + base = refine without re-adding garments (no duplicate files). Superseded as the *editing* path by the **Refine this outfit** action (#24), which is a real promptable edit |
...
| 15 | SVD motion clips (async, queued) | 2026-08-22 | `svd_xt.safetensors` symlinked into the comfyui image (recreate loses the symlink — weight persists). `POST /api/tryon/clip` submits → `GET /api/clips/{id}` polls; runs server-side so navigating away doesn't stop it. `clips` table tracks queued→running→done/error. **UNWIRED FROM THE UI (2026-09-23)** — the outfit card's button became **Refine this outfit** (#24) and the try-on tab's clip control was removed too; the feature is on hold, not deleted (backend + existing clips untouched) |
...
| 22 | DeepSeek-v4-flash API for stylist chat instead of local Ollama text model | 2026-08-23 | Zero VRAM cost — chat offloaded to cloud API keeps full ~14–15GB VRAM headroom for the renderer. `DEEPSEEK_API_KEY` in `.env`; model configurable via `DEEPSEEK_CHAT_MODEL`. Qwen2.5-VL (vision) on Ollama unchanged for garment uploads. |
...
| 24 | **Qwen-Image-2.1 replaces the CatVTON + IDM-VTON stack entirely** | 2026-09-22/23 | Qwen is a general image editor, so the person + each garment go in as references and a whole outfit renders in one pass — no mask/AutoMasker/DensePose stage, no per-piece geometry pass, and none of the repairs that stack needed (face restore, mask∩person halo clipping, deterministic colour matching) because it never re-generated outside its lane. Direct comparison: CatVTON stretched shorts to long pants, drifted colour, shrank outerwear and lost printed graphics; IDM re-textured better but re-generated the whole frame, so it bled colour between garments and still needed a face restore. Qwen held garment colour, sleeve length and print from the reference alone. `cfg=1.0` makes negative prompts inert, so the prompt is positive-only and names each reference **by role only** — injecting `vision_desc` or `name` overrode the reference and produced invented trim (outfits 114/115) and recoloured garments (118). Removal: `tryon.py` 1861 → 707 lines, 3 workflow JSONs + `services/comfyui/` + bootstrap script deleted, ~33 GB of weights reclaimed, base-photo style classification moved from AutoMasker to vision. |
| 25 | **Refine this outfit** — promptable edit on a saved render, saved as a NEW outfit | 2026-09-23 | `POST /api/outfits/{id}/refine` → `tryon.refine_render` sends the render as `<image1>` with no garment refs and applies a written instruction. Replaced the outfit card's **Make a 3s clip** button — an edit the user actually asked for beats a 3-second pan. The original is never overwritten (renders are permanent artifacts), so both versions sit side by side. The prompt holds identity + scene but deliberately does **not** pin the pose, so "turn her to the side" works |
| 26 | **Wardrobe shows the de-backgrounded garment** (`.cutout.png`), served with its alpha intact | 2026-09-23 | A garment photo and its derivations are three different files, and each consumer gets its own: the **original photo** is the truth (rotate / vision / phash / edits operate on it), **`<gid>.clean.png`** is the garment on plain white and is what the renderer sees as a reference (`tryon-pipeline.md` §4), and **`<gid>.cutout.png`** is the true-RGBA cutout the **UI displays**. `media.garment_display_path()` prefers the cutout, `garment_display_path`'s fallback keeps a garment visible when no cutout exists, and `_is_variant()` treats both companions as derived so the `{gid}.*` glob can never hand the cutout to rotate/vision. Two bugs this fixed: (a) a bare `.convert("RGB")` in the WebP variant writer flattened the cutout, exposing the magenta fill Qwen leaves under the transparency (~(165,58,208)) — every wardrobe card was bright purple; alpha is now preserved (`_webp_ready`). (b) `.cutout.png` sorts before `.jpg`, so the cutout had silently become "the original" for every glob-based caller. Display surfaces use the `--imgpanel` slate gradient instead of flat black, so a transparent cutout reads as a product shot instead of a hole. Tests: `tests/test_cutout_display.py` |
...
app/
  ...
  media.py           garment-image serving (original / .clean.png / .cutout.png)
                     + product-fetch + orientation helpers
...
- Qwen-Image-2.1 edit pass: **~60–120s** (1 garment ~60s, 2 ~85–100s, 3-item
  chained ~3m30s). The CatVTON-era "~10–20s/image" figure is gone — the quality
  trade is deliberate.
- Qwen-Image-2.1 background removal (`bg_remove.py` on 202): **~60s per
  garment** at native resolution (25 steps) — 163 garments = ~2.7h, run as a
  batch job, never inline in a request. It is the replacement for the rembg
  step that was only ever planned, not built.
